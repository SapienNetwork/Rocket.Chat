Meteor.methods({
	voiceChannelsList(filter, channelType, limit, sort) {
		this.unblock();
		...
		if (channelType !== 'private') {
			if (RocketChat.authz.hasPermission(Meteor.userId(), 'view-v-room')) {
				roomTypes.push({
					type: 'v'
				});
			} else if (RocketChat.authz.hasPermission(Meteor.userId(), 'view-joined-room')) {
				const roomIds = _.pluck(RocketChat.models.Subscriptions.findByTypeAndUserId('v', Meteor.userId()).fetch(), 'rid');
				roomTypes.push({
					type: 'v',
					ids: roomIds
				});
