Template.listVoiceChannelsFlex.helpers({
	channel() {
...
Template.listVoiceChannelsFlex.events({
	'click header'() {
	...
	'click footer .create'() {
		if (RocketChat.authz.hasAtLeastOnePermission( 'create-v')) {
...
Template.listVoiceChannelsFlex.onCreated(function() {
	...
	return this.autorun(() => {
		if (this.show.get() === 'joined') {
			...
			return Meteor.call('voiceChannelsList', this.nameFilter.get(), 'public', this.limit.get(), this.sortChannels.get(), (err, result) => {
				if (result) {
					this.hasMore.set(true);
