Template.voiceChatRoomItem.helpers({

	alert() {
		if ((FlowRouter.getParam('_id') !== this.rid) || !document.hasFocus()) {
			return this.alert;
		}
	},


	userStatus() {
		let userStatus = RocketChat.roomTypes.getUserStatus(this.t, this.rid);
		return `status-${userStatus || 'offline'}`;
	},

	name() {
		return this.name;
	},

	roomIcon() {
		return RocketChat.roomTypes.getIcon(this.t);
	},

	active() {
		if (Session.get('openedVoiceChannel') === this.rid) {
			return 'active';
		}
	},

	canLeave() {
		let roomData = Session.get(`roomData${this.rid}`);

		if (!roomData) { return false; }

		if (((roomData.cl != null) && !roomData.cl) || (roomData.t === 'd')) {
			return false;
		} else {
			return true;
		}
	},

	route() {
		return RocketChat.roomTypes.getRouteLink(this.t, this);
	},

	isMuted() {
		return (Session.get('muteVoiceChannel')===true);
	}
});


Template.chatRoomItem.rendered = function() {
	if (!((FlowRouter.getParam('_id') != null) && (FlowRouter.getParam('_id') === this.data.rid)) && !this.data.ls && (this.data.alert === true)) {
		return KonchatNotification.newRoom(this.data.rid);
	}
};

Template.voiceChatRoomItem.events({

	'click .mute-microphone'() {
		Session.set('muteVoiceChannel',true);
		return twilioConnection.mute(true);
	},
		//console.log(twilioConnection.isMuted())

	'click .un-mute-microphone'() {
		Session.set('muteVoiceChannel',false);
		return twilioConnection.mute(false);
	},
		//console.log(twilioConnection.isMuted())

	'click .open-room'(e) {
		return menu.close();
	},

	'click .exit-voice-channel'(e) {
		e.stopPropagation();
		e.preventDefault();
		console.log('exit');
		return Meteor.call('exitVoiceChannel');
	},


	'click .hide-room'(e) {
		e.stopPropagation();
		e.preventDefault();

		let { rid } = this;
		let { name } = this;

		let warnText = (() => { switch (this.t) {
			case 'c': return 'Hide_Room_Warning';
			case 'p': return 'Hide_Group_Warning';
			case 'd': return 'Hide_Private_Warning';
			case 'v': return 'Hide_Voice_Warning';
		} })();

		return swal({
			title: t('Are_you_sure'),
			text: warnText ? t(warnText, name) : '',
			type: 'warning',
			showCancelButton: true,
			confirmButtonColor: '#DD6B55',
			confirmButtonText: t('Yes_hide_it'),
			cancelButtonText: t('Cancel'),
			closeOnConfirm: true,
			html: false
		}, function() {
			let needle;
			if ((needle = FlowRouter.getRouteName(), ['channel', 'group', 'direct'].includes(needle)) && (Session.get('openedRoom') === rid)) {
				FlowRouter.go('home');
			}

			return Meteor.call('hideRoom', rid, function(err) {
				if (err) {
					return handleError(err);
				} else {
					if (rid === Session.get('openedRoom')) {
						return Session.delete('openedRoom');
					}
				}
			});
		});
	},

	'click .leave-room'(e) {
		e.stopPropagation();
		e.preventDefault();

		let { rid } = this;
		let { name } = this;

		let warnText = (() => { switch (false) {
			case this.t !== 'c': return 'Leave_Room_Warning';
			case this.t !== 'p': return 'Leave_Group_Warning';
			case this.t !== 'd': return 'Leave_Private_Warning';
			case this.t !== 'v': return 'Leave_Voice_Warning';
		} })();
		return swal({
			title: t('Are_you_sure'),
			text: t(warnText, name),
			type: 'warning',
			showCancelButton: true,
			confirmButtonColor: '#DD6B55',
			confirmButtonText: t('Yes_leave_it'),
			cancelButtonText: t('Cancel'),
			closeOnConfirm: false,
			html: false
		}, function(isConfirm) {
			if (isConfirm) {
				return Meteor.call('leaveRoom', rid, function(err) {
					if (err) {
						return swal({
							title: t('Warning'),
							text: handleError(err, false),
							type: 'warning',
							html: false
						});

					} else {
						let needle;
						swal.close();
						if ((needle = FlowRouter.getRouteName(), ['channel', 'group', 'direct'].includes(needle)) && (Session.get('openedRoom') === rid)) {
							FlowRouter.go('home');
						}

						return RoomManager.close(rid);
					}
				});
			} else {
				return swal.close();
			}
		});
	}
});
